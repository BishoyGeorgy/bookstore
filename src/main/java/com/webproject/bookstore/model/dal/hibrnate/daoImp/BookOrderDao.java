...
public class BookOrderDao extends SuperDao<BookOrder> {
    ...
    public ArrayList<Book> getBooksByBookOrderId(BookOrderId bookOrderId){
    	if (!session.isOpen()) {
    ...
    public ArrayList<Book> getBooksByDeliveryDate(Date date){
    	if (!session.isOpen()) {
            session = SessionManager.getSession();
        }
        Criteria bookCriteria = session.createCriteria(BookOrder.class)
    ...
    public List<BookOrder> getBookOredrsByOrderId(int orderID){
        if (!session.isOpen()) {
            session = SessionManager.getSession();
        }
        session.beginTransaction();
